...
def pensim_reward(z, u):
    '''
    Penicillin yield is a STATE variable (pensimpy's yield_per_run), carried as
    the last coordinate of PenSimSINDyEnv's observation (scaled by YIELD_SCALE).
    So reward is an exact projection, not a function that needs to be fit --
    this is deliberately trivial; the interesting model is the SINDy dynamics
    row for this coordinate, not this function.
    '''
    return 10.0 * z[9]
