...
try:
    ...
    # HydroSVDWrapper is still *defined* at module scope below, so it needs a
    # base class to exist. `object` makes the class definition succeed while
    # ensuring any attempt to actually instantiate it fails loudly.
    FlowEnv = object
    hgym = None

# Don't require that user needs smpl/pensimpy installed
try:
    from sindy_rl.pensim.env import PenSimSINDyEnv
except ImportError:
    warnings.warn('smpl / pensimpy is not installed!')
...
class HydroSVDWrapper(FlowEnv):
    def __init__(self, config):
        if hgym is None:
            raise ImportError('HydroSVDWrapper requires hydrogym.')
